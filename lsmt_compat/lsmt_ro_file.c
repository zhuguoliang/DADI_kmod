...
static bool verify_mapping_order(const struct segment_mapping *pmappings,
  ...
  for (; it != pmappings + n - 1; it++) {
    ...
    if (segment_end(it) <= nt->offset)
      continue;
    PRINT_ERROR("segment disordered. [%lu %lu] , [%lu %lu]", it->offset,
                segment_end(it), nt->offset, segment_end(nt));
    return false;
  }
  return true;
}
  ...
  for (; it != pmappings + n; it++) {
    ...
    if (!(moffset_begin <= it->moffset &&
      ...
      PRINT_ERROR("invalid index moffset: "
                  "[ %lu, %lu ] not in [%lu, %lu]",
                  it->moffset, segment_mapping_mend(it), moffset_begin,
                  moffset_end);
      return false;
    }
  }
...
create_memory_index(const struct segment_mapping *pmappings, size_t n,
  ...
  bool ok1 = verify_mapping_moffset(pmappings, n, moffset_begin, moffset_end);
  ...
  if (ok0 & ok1) {
    ...
    if (!ret) {
#ifndef __KERNEL__
      PRINT_ERROR("malloc memory failed. %d %s", errno, strerror(errno));
#else
...
static bool is_trailer(const struct lsmt_ht *ht) { return !is_header(ht); }
...
static struct segment_mapping *do_load_index(void *fd,
  ...
  if (trailer) {
    ...
    if (pht->index_offset != HT_SPACE) {
      PRINT_ERROR("index offset wrong (fildes: %d)", (int)(uint64_t)fd);
      goto error_ret;
    }
...
merge_memory_indexes(struct lsmt_ro_index **indexes, size_t n) {
  ...
  struct segment_mapping *mappings = (struct segment_mapping *)_lsmt_malloc(
  ...
  ret = (struct lsmt_ro_index *)_lsmt_malloc(sizeof(struct lsmt_ro_index));
  ...
  while (true) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (mapping[i].zeroed == 0) {
        read = _lsmt_pread(fd, data, size, mapping[i].moffset * ALIGNMENT);
        if (read < size) {
#ifndef __KERNEL__
